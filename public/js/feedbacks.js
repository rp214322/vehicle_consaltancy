var feedbackTable = $("#FeedbackTable").DataTable({
  ...
  columns: [
    ...
    { data: "user", name: "user.first_name", orderable: true, width: "10%" },
...
jQuery(function() {
        ...
        error: function(error) {
          ...
          $.each(error.responseJSON, function(key, value) {
            ...
            jQuery(_f)
              .find(input)
              .parent()
              .append("<span class='has-error'>" + value + "</span>");
          });
        }
      };
      ajax.fire();
  ...
  $(document).on("click", ".change_status", function() {
    ...
    $.ajax({
      url: updateStatus, // Your route to update status
      type: "POST",
      dataType: "json",
      headers: {
      ...
      success: function(response) {
        feedbackTable.ajax.reload(null, false); // Reload table to reflect changes
      },
      error: function(xhr) {
